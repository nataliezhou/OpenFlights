...
vector<Airport*> BFS::bfs_all_airports(int start_idx) { 
    ...
    while (!queue.empty()) {
        ...
        to_return.push_back(graph_->getAirport(curr_idx));
        visited_idx[curr_idx] = true;
        for (unsigned long col = 1; col <= adj_matrix.size(); col++) { // because id of airports start at 1
            ...
            if (adj_matrix[curr_idx][col] != 0 && !visited_idx[col]) queue.push(col); 
        }
    }
    return to_return;
}
